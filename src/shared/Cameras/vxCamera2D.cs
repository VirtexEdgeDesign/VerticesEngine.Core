...
{
	public class vxCamera2D : vxCamera
	{
		...
		public vxCamera2D(vxGameplaySceneBase sceneBase) : base(sceneBase)
        {

			SimProjection = Matrix.CreateOrthographicOffCenter(0f, ConvertUnits.ToSimUnits(vxGraphics.GraphicsDevice.Viewport.Width), ConvertUnits.ToSimUnits(vxGraphics.GraphicsDevice.Viewport.Height), 0f, 0f, 1f);
			SimView = Matrix.Identity;
			View = Matrix.Identity;

            var w = vxGraphics.GraphicsDevice.Viewport.Width;
            var h = vxGraphics.GraphicsDevice.Viewport.Height;

            Projection = Matrix.CreateOrthographicOffCenter(0f, w, h, 0f, 0f, 1f);

            vxConsole.WriteLine("2D Camera -> W: " + w + ", H:" + h);

			_translateCenter = new Vector2(ConvertUnits.ToSimUnits(w / 2f), ConvertUnits.ToSimUnits(h / 2f));
        ...
        //    //Renderer.RenderingPasses.Add(new vxSceneBlurPostProcess(Renderer));
        //}
        ...
        /// </summary>
        /// <value>The safe area viewport.</value>
        public Rectangle SafeAreaViewport
        {
            get
            {
                return _safeAreaViewport;
            }
        }
        Rectangle _safeAreaViewport;

        /// <summary>
        /// The viewport safe area padding.
        /// </summary>
        public int ViewportSafeAreaPadding = 50;
        ...
        public void ShakeChamera(float Amount, float ShakeLength)
        {
            _shakeCamera = true;
            _shakeAmount = Amount;

            _shakeLength = ShakeLength;
        }

        float _shakeLength = 8;
        float _shakeAmount = 0;
        bool _shakeCamera = false;
		...
		private void SetView()
		{
			...
			Matrix matZoom = Matrix.CreateScale(_zoom);
		...
		protected internal override void Update()
		{
            ...
            float Width = (vxGraphics.GraphicsDevice.Viewport.Bounds.Width - ViewportSafeAreaPadding) / this.Zoom;
            float Height = (vxGraphics.GraphicsDevice.Viewport.Bounds.Height - ViewportSafeAreaPadding) / this.Zoom;

            Point bndpos = new Point(
                (int)(this.Position.X - Width / 2), 
                (int)(this.Position.Y - Height / 2));


            _safeAreaViewport = new Rectangle(
                bndpos, new Point((int)(Width), (int)(Height))
            );
            ...
            _currentPosition += CameraTargetSpringStiffness * delta * inertia * vxTime.DeltaTime;
			_currentRotation += 80f * rotDelta * rotInertia * vxTime.DeltaTime;


            // Get off screen bounds
            if (_shakeCamera == true)
            {
                if (_shakeAmount > 0.01f)
                {
                    _shakeAmount = vxMathHelper.Smooth(_shakeAmount, 0, _shakeLength);
                    runningShake += _shakeAmount;

                    MoveCamera(_shakeAmount * Vector2.Transform(Vector2.UnitX, Matrix.CreateRotationZ(runningShake*1)));
                    //SetView();
                }
                else
                    _shakeCamera = false;
        ...
        public float CameraTargetSpringStiffness = 100.0f;

		public Vector2 ConvertScreenToWorld(Vector2 location)
		{
			Vector3 t = new Vector3(location, 0);
			t = vxGraphics.GraphicsDevice.Viewport.Unproject(t, SimProjection, SimView, Matrix.Identity);
		...
		{
			Vector3 t = new Vector3(location, 0);
			t = vxGraphics.GraphicsDevice.Viewport.Project(t, SimProjection, SimView, Matrix.Identity);
			return new Vector2(t.X, t.Y);
		}
	}
}