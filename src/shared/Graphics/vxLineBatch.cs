...
namespace VerticesEngine.Graphics
{
	...
	public class vxLineBatch : IDisposable
	{
        ...
        {
            Vector2 loc = rect.Location.ToVector2();

            DrawLine(loc, loc + new Vector2(rect.Width, 0), color);

            DrawLine(loc, loc + new Vector2(0, rect.Height), color);

            DrawLine(loc + new Vector2(rect.Width, 0), loc + new Vector2(rect.Width, rect.Height), color);


            DrawLine(loc + new Vector2(0, rect.Height), loc + new Vector2(rect.Width, rect.Height), color);
        }
        ...
        public void DrawLine(Vector2 v1, Vector2 v2, Color color, float z)
        {
			if (!_hasBegun)
				throw new InvalidOperationException("Begin must be called before DrawLineShape can be called.");
			...
			_lineVertices[_lineVertsCount].Position = new Vector3(v1, z);
			_lineVertices[_lineVertsCount + 1].Position = new Vector3(v2, z);
			_lineVertices[_lineVertsCount].Color = _lineVertices[_lineVertsCount + 1].Color = color;
			_lineVertsCount += 2;
        }
